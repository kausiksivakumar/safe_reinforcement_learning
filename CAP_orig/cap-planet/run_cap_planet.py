...
observation_model = ObservationModel(int(args.symbolic_env), int(env.observation_size), int(args.belief_size), int(args.state_size), int(args.embedding_size), args.activation_function).to(device=args.device)
...
encoder = Encoder(args.symbolic_env, int(env.observation_size), int(args.embedding_size), args.activation_function).to(device=args.device)
...
betas = (0.9, 0.999)
policy_lr = 0.001
policy = PPO(args.state_size, env.action_size, policy_lr, action_std, betas, args.cost_discount, args.discount, args.device)#.to(args.device)
#########################################################################################################################################
