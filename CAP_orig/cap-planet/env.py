...
from safety_envs import load_suite_env, CONSTRAINED_GYM_ENVS, CONSTRAINED_CONTROL_SUITE_ENVS

GYM_ENVS = ['Safexp-PointGoal1-v0','Pendulum-v0', 'MountainCarContinuous-v0', 'Ant-v2', 'HalfCheetah-v2', 'Hopper-v2', 'Humanoid-v2', 'HumanoidStandup-v2', 'InvertedDoublePendulum-v2', 'InvertedPendulum-v2', 'Reacher-v2', 'Swimmer-v2', 'Walker2d-v2', 'CarRacing-v0', *CONSTRAINED_GYM_ENVS]
CONTROL_SUITE_ENVS = ['cartpole-balance', 'cartpole-swingup', 'reacher-easy', 'finger-spin', 'cheetah-run', 'ball_in_cup-catch', 'walker-walk', *CONSTRAINED_CONTROL_SUITE_ENVS]
...
class ControlSuiteEnv():
  def __init__(self, env, symbolic, seed, max_episode_length, action_repeat, bit_depth, cost_reduce="max"):
    from dm_control.suite.wrappers import pixels
    domain = env.split('-')[0]
    self._env = load_suite_env(env, seed)
    ...
    if cost_reduce == "sum":
        self.cost_reduce = sum
    if cost_reduce == "max":
        self.cost_reduce = max
  ...
  def step(self, action):
    ...
    for k in range(self.action_repeat):
      ...
      reward += state.reward
      cost = self.cost_reduce([getattr(state, 'cost', 0), cost])
...
class GymEnv():
  ...
  def step(self, action):
    ...
    for k in range(self.action_repeat):
      ...
      reward += reward_k
      cost = self.cost_reduce([info.get('cost', 0), cost])
      self.t += 1  # Increment internal timer
...
class ClipActionWrapper(Wrapper):
    def __init__(self, env):
        super(ClipActionWrapper, self).__init__(env)
...
def Env(env_name, symbolic, seed, max_episode_length, action_repeat, bit_depth, cost_reduce="max"):
  if env_name in GYM_ENVS:
    env = GymEnv(env_name, symbolic, seed, max_episode_length, action_repeat, bit_depth, cost_reduce=cost_reduce)
  elif env_name in CONTROL_SUITE_ENVS:
    env = ControlSuiteEnv(env_name, symbolic, seed, max_episode_length, action_repeat, bit_depth, cost_reduce=cost_reduce)
  else:
    env = None
